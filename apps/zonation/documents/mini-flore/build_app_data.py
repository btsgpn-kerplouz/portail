...
# Deux CSV : les marais salés (Colasse 2019) et les milieux dunaires
# (Delassus et al. 2018). Meme format de colonnes, meme traitement.
CSV_PATHS = [
    HERE.parent / "especes_zonation_illustrations(URL).csv",
    HERE.parent / "especes_dunes_illustrations(URL).csv",
]
...
def main():
    ...
    for csv_path in CSV_PATHS:
        if not csv_path.exists():
            print("Absent, ignore :", csv_path.name)
            continue
        rows = list(csv.reader(csv_path.open(encoding="utf-8-sig"), delimiter=";"))
        for r in rows[1:]:
            # Le CSV dunaire n'a pas toujours la 10e colonne remplie : on exige
            # seulement d'atteindre la premiere colonne de photo.
